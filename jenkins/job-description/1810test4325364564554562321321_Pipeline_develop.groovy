...
def job_config = [
        job: [
                name: "1810test4325364564554562321321_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: '1810test4325364564554562321321_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/1810test4325364564554562321321_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.1810test4325364564554562321321.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "jarro06", "1810test4325364564554562321321", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("1810test4325364564554562321321_Pipeline_develop");

job.with {

    authenticationToken('oZ7oMKIsX8769VMDTaZzxoh6ziu2xRWI')
